...
- 이탤릭(*...*), 볼드(**...**), 취소선(~~...~~) 강조 금지. 모든 문단은 일반 텍스트.
- 물결표(~) 사용 금지. 숫자 범위는 "6에서 8시간", "6-8시간"처럼 쓰세요. "6~8"처럼 쓰지 마세요.
- 참고 문헌 섹션 금지.
...
- 외부 이미지 마크다운 (![...](...)) 및 출처 표기
- 물결표(~) 사용: "6~8" → "6-8" 또는 "6에서 8"로 교체. 취소선(~~...~~)도 제거.
...
export const generatePost = onCall(
  ...
  async (request) => {
    ...
    // 마크다운 후처리: 외부 이미지, 취소선, 볼드, 이탤릭, 물결표 제거
...
- 이탤릭, 볼드, 취소선 서식 금지. 일반 텍스트만.
- 물결표(~) 사용 금지. 범위는 "-"이나 "에서"로 표기.
- 문단과 문단 사이에 빈 줄을 넣으세요
